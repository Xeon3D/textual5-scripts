...
property CodeName : "Monterey fixes..."
property SupportChannel : "irc://irc.libera.chat/#textual"
...
on textualcmd(cmd)
	...
	-- Defines POSIX Path to Textual 7's Macintosh Models List
	set ClientName to name of current application
	if ClientName contains "Textual" then
		tell application "Finder"
			set apppath to path to current application as text
		end tell
		set plistpath to quoted form of ((POSIX path of apppath) & "Contents/Resources/Bundled\\ Extensions/SystemProfiler.bundle/Contents/Resources/MacintoshModels.plist")
	end if
	...
	set montereytest to do shell script "sw_vers | grep ProductVersion | awk -F':' {'print $2'}"
	if montereytest contains "12." then
		set isMonterey to true
	else
		set isMonterey to false
	end if
	...
	if ViewMac then
		...
		try
			if isMonterey is false then
				set AppleResponse to do shell script "curl https://support-sp.apple.com/sp/product?cc=G8WN&land=en_US"
			else
				set AppleResponse to do shell script "curl https://support-sp.apple.com/sp/product\\?cc\\=" & SNIdentifier & "\\&land\\=en_US"
			end if
		...
		on error
			try
				set machineName to do shell script "defaults read " & plistpath
			on error
				set machineName to ""
			end try
		end try
	...
	if ViewDisk then
		...
		tell application "System Events"
			set allVolumesList to (name of every disk whose local volume is true and ejectable is false and format is Mac OS Extended format or format is NFS format) as list
			if (count of allVolumesList) is less than 1 then
		...
		set FreeSpace to ""
		-- Disks to remove
		set DisksToDelete to {"VM", "Preboot", "Update", "home", "mnt1"}
		set volumesList to {}
		repeat with i from 1 to count allVolumesList
			if {allVolumesList's item i} is not in DisksToDelete then set volumesList's end to allVolumesList's item i
		end repeat
		if isMonterey is false then
			...
			repeat with volume in volumesList
				--		set diskTotals to "1000211607552
				--460070924288"
				set testforcontainer to do shell script "diskutil info " & the quoted form of volume
				if testforcontainer contains "Container" then
					set diskTotals to do shell script "diskutil info " & the quoted form of volume & " | awk '/Container Total Space|Container Free Space/' | awk -F':' {'print $2'} | tr -d ' ' | awk -F'(' {'print $2'} | awk -F'B' {'print $1'}"
				else
					set diskTotals to do shell script "diskutil info " & the quoted form of volume & " | awk '/Total Space|Volume Free Space/' | awk -F':' {'print $2'} | tr -d ' ' | awk -F'(' {'print $2'} | awk -F'B' {'print $1'}"
				end if
				set TotalSpaceD to roundThis((the first paragraph of diskTotals as integer) / 1.0E+9, 1)
				set FreeSpaceD to roundThis((the second paragraph of diskTotals as integer) / 1.0E+9, 1)
				set TotalSpace to TotalSpace + TotalSpaceD
	...
	if ViewPower then
		try
			...
			if isMonterey is false then
				...
				set PowerInfo to do shell script "pmset -g ps | head -n2 | tail -n1 | awk {'print $3,$4,$5'}"
			end if
			...
			if BatteryStatus is "charging" then
				...
				set BatteryCapacity to BatteryCapacity & " (Not charging)"
			end if
	...
	if ViewOSXVersion then
		...
		set OSVersion to item 2 of OSXInfo
		set AppleScript's text item delimiters to "."
		set majorversion to text item 1 of OSVersion
		set minorversion to text item 2 of OSVersion
		set AppleScript's text item delimiters to ""
		if majorversion = 10 and minorversion < 12 then
			set OSName to "Mac OS X"
		else
			set OSName to "macOS"
		end if
		set msg to msg & FBold & "OS: " & FBold & OSName & " " & item 2 of OSXInfo & " "
		set AppleScript's text item delimiters to ""
