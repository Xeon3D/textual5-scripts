...
-- 2 step Installation:
-- Copy this to ~/Library/Application Scripts/com.codeux.irc.textual5
...
property CurrentVersion : "0.5.7"
property CodeName : "Mahna Mahna!"
...
on textualcmd(cmd)
	...
	--HDD
	if ViewDisk then
		set TotalInfo to do shell script "diskutil info / | grep \"Total Size:\" | awk {'print $3,$4'}"
		set TotalSpace to first word of TotalInfo as real
		set TotalSpaceUnit to second word of TotalInfo
		set FreeInfo to do shell script "diskutil info / | grep \"Free Space:\" | awk {'print $4,$5'}"
		set FreeSpace to first word of FreeInfo as real
		set FreeSpaceUnit to second word of FreeInfo
		set UsedSpace to TotalSpace - FreeSpace
		set UsedSpaceUnit to TotalSpaceUnit
		set AllDisksUsedPercentage to round (((TotalSpace - FreeSpace) / TotalSpace) * 100)
	...
	if ViewClient then
		...
		if ViewClientBuild then
			...
			set TextualBuild to do shell script "defaults read " & InfoPlist & " | grep TXBundleBuildCodeName | awk -F\\\" {'print $2'}"
